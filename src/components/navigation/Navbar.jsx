  ...
  return (
    <nav className={`navbar justify-between z-50 shadow-md border-y-stone-900 border-y-4 fixed flex h-[14vh] top-0 left-0 mx-auto w-[100vw] px-2 sm:px-6 lg:px-8 ${scrollNav ? 'scrollNav' : ''}`}>
        ...
        <ul className=" flex items-center space-x-6 rounded-lg border-slate-200 border-2 rounded-md h-[50px] mt-[18px] w-[35vw] justify-center" ref={linkRef}>
          {links.map((link) => (
            <li className={` border-slate-200 border-2 rounded-md px-1.5 mb-[130px] ${link.class}`} onMouseEnter={onEnter} onMouseLeave={onLeave} key={link.text}>
              <NavLinks to={link.path} smooth duration={500} spy exact="true">{link.text}</NavLinks>
            </li>
          ))}
        </ul>
      </section>
      <div className={navBar}>
        <HiMenuAlt4
          onClick={toggleMenu}
          className={`absolute mt-[4vh] text-3xl${isMenuOpen ? 'w-[0] h-[]0' : ' '}`}
        />
      </div>
      <section className={`${isMenuOpen ? 'absolute nav_ul' : 'pointer-events-none'}`}>
        <div className={`${breakPoint > 1242 ? 'hidden' : 'h-[95vh] w-[75vw]'} `}>
          <motion.ul
            ...
            className={`grid grid-row-4 mobileNav pt-[7vh] rounded-lg border-x-stone-900 border-x-4 h-[94vh] w-[70vw] justify-center transition-transform ease-in-out duration-500 ${isMenuOpen ? '' : 'hidden'}`}
          >
            ...
            {links.map((link) => (
              <motion.li
                ...
                className="btnNav hover:scale-125 border-slate-200 border-2 rounded-md h-[6vh] w-[200px] text-center"
                key={link.text}
              >
                <NavLinks to={link.path} smooth duration={500} spy exact="true" onClick={toggleMenu} className="btn_link_mobile">{link.text}</NavLinks>
              </motion.li>
            ))}
          </motion.ul>
        </div>
